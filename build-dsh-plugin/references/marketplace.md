...
- `compatibility.dshReleases` explicitly marks every full version in that live window as `compatible`, `incompatible`, or `unknown`; an approved proposal needs at least one exact `compatible` result. The 2026-09-02 window is `0.1.2-alpha.2`, `0.1.2-alpha.3`, and `0.1.2-alpha.4`, but must be refreshed rather than copied forward;
