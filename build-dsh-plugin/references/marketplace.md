...
- `compatibility.dshReleases` explicitly marks every full version in that live window as `compatible`, `incompatible`, or `unknown`; an approved proposal needs at least one exact `compatible` result. The 2026-09-07 window is `0.1.3-alpha.2`, `0.1.5-alpha.1`, and `0.1.5-alpha.2`, but must be refreshed rather than copied forward;
