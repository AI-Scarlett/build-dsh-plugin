...
- `compatibility.dshReleases` explicitly marks every full version in that live window as `compatible`, `incompatible`, or `unknown`; an approved proposal needs at least one exact `compatible` result. The 2026-09-05 window is `0.1.2-alpha.4`, `0.1.2-alpha.5`, and `0.1.2-rc.1`, but must be refreshed rather than copied forward;
