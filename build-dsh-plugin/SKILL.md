...
Do not wait until submission to discover listing incompatibility. From the first scaffold, reserve unique package/catalog/entry IDs; declare repository, version, license, safe `dsh.bundle.patch`, package-relative Patch, runtime/build files, lifecycle scripts, permissions, dependencies, and compatibility evidence. At the start of every STORE assessment, run `node scripts/official-dsh-releases.mjs` and use its official latest-three window; do not reuse a remembered window. The resolver follows the published official GitHub Releases and npm version records in the active major.minor series, excludes drafts, unsupported future previews and deprecated npm versions, and fails closed on invalid authority. The observed 2026-09-07 window is `0.1.3-alpha.2`, `0.1.5-alpha.1`, and `0.1.5-alpha.2`, but it is evidence rather than a permanent constant. Historical `0.1.2-rc.1` remains documented as a previously verified release. Keep exact `compatibility.dshReleases` and matching `compatibility.dshOperations` records for install, start, uninstall, and rollback on every release in the resolved window. Historical keys may remain, but unknown metadata stays unknown instead of being guessed.
