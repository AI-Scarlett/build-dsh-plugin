...
Use [catalog-entry.template.json](assets/catalog-entry.template.json), but derive its values from the pinned repository and current Registry contract. Include fixed-source freshness, four assurance levels, and per-release operation evidence from rc.5 through `0.1.1-rc.2`, retaining the `0.1.1-rc.1` history key. A local trusted candidate can be `READY_FOR_PINNED_SOURCE_VERIFICATION`; only current STORE validation plus fixed-Commit source verification can make it PR-ready. Only a merged remote catalog and public page readback prove actual listing.
