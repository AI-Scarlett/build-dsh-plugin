...
`build-dsh-plugin` 0.4.1 是面向 DeepSeek Harness（DSH）插件开发的 Agent Skill，并提供原生 Skill Provider Bundle：DSH `0.1.2-alpha.3`、`alpha.4` 与 `alpha.5` 的公开接口兼容已核对，并通过隔离安装、配置合成与冷启动。用户只需说明“问题、结果、成功标准”，即可生成标准 Bundle、官方最新三个 DSH 版本的动态兼容和操作证据矩阵、权限矩阵、测试、审计和验收流程。
