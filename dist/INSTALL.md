...
- `build-dsh-plugin-20260903.1.zip`：完整 Skill，ZIP 顶层是 `build-dsh-plugin/`。
- `build-dsh-plugin-20260903.1.sha256`：ZIP 完整性校验。
- `manifest.json`：版本、入口、依赖、校验状态和明确排除项。

固定发行页：<https://github.com/AI-Scarlett/build-dsh-plugin/releases/tag/v2026.09.03.1>
...
unzip -l build-dsh-plugin-20260903.1.zip
```

期望 SHA-256：

```text
39cdc334706e72773b6b87b4893e45aa9947d24c2da931a69b7d793165610512
```
...
当前最新三个 DSH 版本由官方 npm `latest`/`alpha`/`beta`/`rc` 通道动态解析。仓库根目录的 `dsh-build-plugin` 0.4.1 Bundle 已核对 `0.1.2-alpha.3`、`alpha.4` 与 `alpha.5` 的公开接口，并在三个版本的一次性 Profile 中通过安装、配置合成和冷启动；`alpha.5` 还完成运行时 Skill 发现、卸载与回滚复装。声明范围仍为 `>=0.1.0-rc.8 <0.2.0`。Bundle 通过唯一条目 `dsh-build-plugin-skill-provider` 隔离挂载同一份 Skill，不安装 Codex、Claude 或 Grok Runtime，也不覆盖官方 Skill Provider。固定 GitHub Commit 的远程/商城入口需要在对应 Commit 合并并通过独立 Registry 校验后使用。
