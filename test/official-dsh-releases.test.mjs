...
test('official release window follows the highest supported channel and excludes next-only versions', () => {
  ...
  assert.equal(window.latestVersion, '0.1.2-alpha.4')
  assert.equal(window.releaseTag, 'alpha')
  assert.deepEqual(window.releases, ['0.1.2-alpha.2', '0.1.2-alpha.3', '0.1.2-alpha.4'])
  assert.ok(!window.releases.includes('0.1.3-next.1'))
})

test('deprecated releases cannot satisfy the latest-three window', () => {
  const value = metadata()
  value.versions['0.1.2-alpha.2'] = { deprecated: 'withdrawn' }
  assert.deepEqual(officialDshReleaseWindow(value).releases, ['0.1.1-rc.2', '0.1.2-alpha.3', '0.1.2-alpha.4'])
})
